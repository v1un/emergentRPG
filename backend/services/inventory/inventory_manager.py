...
from models.game_models import Character, InventoryItem, WorldState
...
class InventoryManager:
    ...
    async def add_item(self, session_id: str, item: InventoryItem, 
                      inventory: List[InventoryItem]) -> InventoryUpdate:
    ...
    async def equip_item(self, session_id: str, item_id: str,
                        inventory: List[InventoryItem]) -> EquipmentUpdate:
        """Equip an item"""
        try:
            item_to_equip = None
            for item in inventory:
            ...
            if not item_to_equip:
                return EquipmentUpdate(False, None, None, "Item not found in inventory")
            
            # Check if item can be equipped
            if item_to_equip.type not in ["weapon", "armor", "accessory"]:
                return EquipmentUpdate(False, None, None, "Item cannot be equipped")
            
            # Find currently equipped item of same type
            currently_equipped = None
            for item in inventory:
                if (item.equipped and 
                    item.type == item_to_equip.type and 
                    item.id != item_id):
    ...
    async def _apply_item_effects(self, item: InventoryItem, character: Character) -> Dict[str, Any]:
        """Apply effects of using an item"""
        effects = {}
