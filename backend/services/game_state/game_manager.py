...
from models.game_models import GameSession, Character, StoryEntry, ActionType, Quest, InventoryItem
...
class GameStateManager:
    ...
    async def handle_player_action(self, session_id: str, action: str, lorebook: Optional[Lorebook] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            result = await realtime_gameplay_flow.execute_full_turn(action, session, lorebook)
    ...
    async def _handle_level_up(self, session: GameSession):
        ...
        try:
            ...
            logger.info(f"Character {session.character.name} leveled up from {old_level} to {session.character.level}")
    ...
    async def _check_story_milestones(self, session: GameSession):
        ...
        try:
            ...
            if story_length == 50:
                ...
                milestone_entry = StoryEntry(type=ActionType.SYSTEM, text=milestone_message)
    ...
    async def get_session_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            total_story_length = sum(len(session.story) for session in self.active_sessions.values())
            avg_story_length = total_story_length / active_count if active_count > 0 else 0
